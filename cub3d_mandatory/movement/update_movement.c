...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/26 13:53:23 by dfasius           #+#    #+#             */
/*   Updated: 2025/05/26 13:53:24 by dfasius          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	movement_ws(t_var *data, t_movestat *movestat)
{
	if (data->move.move_w)
	{
		if (is_valid_movement(data->map.arr[movestat->ipy]
				[movestat->ipx_add_xo]))
			data->player.px += data->player.pdx;
		if (is_valid_movement(data->map.arr[movestat->ipy_add_yo]
				[movestat->ipx]))
	...
	if (data->move.move_s)
	{
		if (is_valid_movement(data->map.arr[movestat->ipy]
				[movestat->ipx_sub_xo]))
			data->player.px -= data->player.pdx;
		if (is_valid_movement(data->map.arr[movestat->ipy_sub_yo]
				[movestat->ipx]))
			data->player.py -= data->player.pdy;
	}
}
		...
		int strafe_ipx, int strafe_ipy)
{
	if (data->move.move_d)
	{
		...
		if (is_valid_movement(data->map.arr[movestat->ipy][strafe_ipx]))
			data->player.px -= data->player.pdy;
		if (is_valid_movement(data->map.arr[strafe_ipy][movestat->ipx]))
