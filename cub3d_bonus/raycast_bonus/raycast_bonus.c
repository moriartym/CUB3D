...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/26 13:48:13 by dfasius           #+#    #+#             */
/*   Updated: 2025/05/26 13:48:15 by dfasius          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_rayhit(t_var *data, t_ray *ray, t_rayhit *rayhit, int tile_size)
{
	...
	rayhit->p_offset_x = (data->player.px - rayhit->p_tile_x * TILE_SIZE)
		/ TILE_SIZE;
	rayhit->p_offset_y = (data->player.py - rayhit->p_tile_y * TILE_SIZE)
		/ TILE_SIZE;
	rayhit->draw_px = (rayhit->p_tile_x - data->minimap_offset_x)
		* tile_size + rayhit->p_offset_x * tile_size;
	rayhit->draw_py = (rayhit->p_tile_y - data->minimap_offset_y)
		* tile_size + rayhit->p_offset_y * tile_size;
	rayhit->r_tile_x = (int)(ray->rx / TILE_SIZE);
	rayhit->r_tile_y = (int)(ray->ry / TILE_SIZE);
	rayhit->r_offset_x = (ray->rx - rayhit->r_tile_x * TILE_SIZE)
		/ TILE_SIZE;
	rayhit->r_offset_y = (ray->ry - rayhit->r_tile_y * TILE_SIZE)
		/ TILE_SIZE;
	rayhit->draw_rx = (rayhit->r_tile_x - data->minimap_offset_x)
		* tile_size + rayhit->r_offset_x * tile_size;
	rayhit->draw_ry = (rayhit->r_tile_y - data->minimap_offset_y)
		* tile_size + rayhit->r_offset_y * tile_size;
}
